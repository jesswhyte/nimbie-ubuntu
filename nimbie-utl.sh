...
while [ "s_status" != "+S14" ]; do
	...
	# Display cd Info
	echo "---------------------CD INFO-----------------------"  
	cdinfo=`isoinfo -d -i /dev/sr1` 
	echo $cdinfo						
	echo "----------------------------------------------------"
	
	## Set Volume name as variable
	volume=`$cdinfo | grep "^Volume id:" | cut -d ":" -f 2` # gets volume name
	...
	echo $cdinfo >> $path/$time-$volume.log
	
	## Get Block size of CD  ##  NOTE: IF the CD is NOT AN ISO, IT IS REJECTED! 
	
	blocksize=`$cdinfo | grep "^Logical block size is:" | cut -d ":" -f 2 | tr -d '[:space:]'` # gets blocksize from isoinfo output
	...
	blockcount=`$cdinfo | grep "^Volume size is:" | cut -d ":" -f 2 | tr -d '[:space:]'` # gets blockcount from isoinfo output
